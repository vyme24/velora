...
export async function POST(req: NextRequest) {
  ...
  const user = await User.findById(parsed.data.userId).select("_id email name accountStatus");
  if (!user || user.accountStatus !== "active") return fail("Unauthorized", 401);

  const ip = req.headers.get("x-forwarded-for") || "unknown";
  const userKey = `resend:login:${String(user._id)}`;
  const ipKey = `resend:login:ip:${ip}`;
